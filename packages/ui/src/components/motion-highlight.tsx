...
type MotionHighlightContextBase = {
...
const MotionHighlightContext = React.createContext<
  MotionHighlightContextBase | undefined
>(undefined)

function useMotionHighlight<T extends string>(): MotionHighlightContextBase & {
  activeValue: T | null
  ...
  return context as MotionHighlightContextBase & {
    activeValue: T | null
    setActiveValue: (value: T | null) => void
  }
}
...
function MotionHighlight<T extends string>({
  ...
  const safeSetActiveValue = (id: string | null) => {
    setActiveValue((prev) => (prev === id ? prev : (id as T | null)))
...
function getNonOverridingDataAttributes(
  ...
  return Object.keys(dataAttributes).reduce<DataAttributes>((acc, key) => {
    const props = element.props as Record<string, React.ReactNode>
...
function MotionHighlightItem({
  ...
  // Use the external helper to assign refs safely
  const handleRef = (node: HTMLDivElement | null) => {
    localRef.current = node
    assignRef(ref, node)

    const childProps = element.props as { ref?: React.Ref<HTMLDivElement> }
    const childElement = element as React.ReactElement & {
      ref?: React.Ref<HTMLDivElement>
    }
    const childRef = childProps.ref ?? childElement.ref
  ...
  return enabled ? (
    <div
      ...
      {...(dataAttributes as Record<string, string | number | boolean>)}
      {...props}
      {...commonHandlers}
    >
      {mode === "children" && (
